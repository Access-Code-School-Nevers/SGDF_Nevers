...
*/
  ...
  public function getAvailablesObjets($date){
    ...
    if(count($tmpDate) == 3 && checkDate($tmpDate[1],$tmpDate[2],$tmpDate[0])){
      ...
      $sql = "SELECT id, titre, perissable, sum(qty) as quantite, GROUP_CONCAT(id_article) as id_article
              FROM (
                  SELECT O.id, titre, perissable, count(*) as qty, GROUP_CONCAT(A.id) as id_article
                  FROM objet O
